import pytest
import numpy as np
from quanestimation.base.AsymptoticBound.AnalogCramerRao import HCRB, NHB
...
def test_HCRB_NHB() -> None:
    ...
    rho = psi @ psi.conj().T
    W = np.array([[1.0, 0], [0, np.sin(theta) ** 2]])
...
def test_HCRB_NHB_invalid_input() -> None:
    ...
    W = np.array([[1.0, 0], [0, 1]])  # Dummy weight matrix
...
def test_HCRB_print(capfd):
    ...
    assert (
        "In single parameter scenario, HCRB is equivalent to QFI. Returning QFI value."
        in out
    )

    # Rank-one weight matrix case
    drho2 = [np.array([[1, 0], [0, 0]]), np.array([[0, 1], [1, 0]])]
    W1 = np.array([[1.0, 0], [0, 0]])  # Rank-one weight matrix
    HCRB(rho, drho2, W1)
    out, _ = capfd.readouterr()
    assert (
        "For rank-one weight matrix, HCRB is equivalent to QFIM. Returning Tr(W @ inv(QFIM))."
        in out
    )
