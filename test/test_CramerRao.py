...
def test_CramerRao():
    ...
    # check the results
    assert (
        np.allclose(result, np.array([[4.0, 0.0], [0.0, np.sin(2 * theta) ** 2]])) == 1
    )
    assert np.allclose(resultc, np.array([[4.0, 0], [0.0, 0.0]])) == 1
...
def test_QFIM_Bloch():
    ...
    assert (
        np.allclose(
            result,
            np.array([[4.0 * eta**2, 0.0], [0.0, eta**2 * np.sin(2 * theta) ** 2]]),
        )
        == 1
    )
