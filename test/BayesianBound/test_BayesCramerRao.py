...
def test_bayesian_bound_singleparameter() -> None:
    ...
    hamiltonian_func = (
        lambda x: 0.5 * b_val * omega0 * (sigma_x * np.cos(x) + sigma_z * np.sin(x))
    )
    ...
    prob_density = lambda x, mu, eta: np.exp(-((x - mu) ** 2) / (2 * eta**2)) / (
        eta * np.sqrt(2 * np.pi)
    )
    ...
    d_prob_values = np.array(
        [d_prob_density(x_val, mu_val, eta_val) for x_val in x_values]
    )
    ...
    # Test BCFIM
    cfim = BCFIM(
        [x_values], prob_normalized, final_states, d_final_states, M=[], eps=1e-8
    )
    ...
    with pytest.raises(TypeError):
        cfim = BCFIM(
            [x_values], prob_normalized, final_states, d_final_states, M=1.0, eps=1e-8
        )
    ...
    # Test BCRB type 1
    bcrb1 = BCRB(
        [x_values], prob_normalized, [], final_states, d_final_states, M=[], btype=1
    )
    ...
    # Test BCRB type 2
    bcrb2 = BCRB(
        [x_values], prob_normalized, [], final_states, d_final_states, M=[], btype=2
    )
    ...
    # Test BQCRB type 1
    bqcrb1 = BQCRB(
        [x_values], prob_normalized, [], final_states, d_final_states, btype=1
    )
    ...
    # Test BQCRB type 2
    bqcrb2 = BQCRB(
        [x_values], prob_normalized, [], final_states, d_final_states, btype=2
    )
    ...
    qvtb = QVTB(
        [x_values], prob_normalized, d_prob_normalized, final_states, d_final_states
    )
    ...
    with pytest.raises(ValueError):
        BCRB(
            [x_values], prob_normalized, [], final_states, d_final_states, M=[], btype=4
        )
...
def test_bayesian_bound_multiparameter() -> None:
    ...
    expected_cfim = np.array([[0.0360, 0.0], [0.0, 2.1649]])
    assert np.allclose(cfim, expected_cfim, atol=1e-3)
    ...
    expected_qfim = np.array([[0.0948, 0.0], [0.0, 3.3352]])
    assert np.allclose(qfim, expected_qfim, atol=1e-3)
    ...
    expected_bcrb1 = np.array([[188.85062035, 0.63311697], [0.63311697, 0.62231953]])
    assert np.allclose(bcrb1, expected_bcrb1, atol=1e-3)
    ...
    expected_bcrb2 = np.array([[27.7234240, 0.002116], [0.002116, 0.461910]])
    assert np.allclose(bcrb2, expected_bcrb2, atol=1e-3)
    ...
    expected_bcrb3 = np.array([[2.52942056, -0.00943802], [-0.00943802, 0.38853841]])
    assert np.allclose(bcrb3, expected_bcrb3, atol=1e-3)
    ...
    expected_vtb = np.array([[0.04382, 0.0], [0.0, 0.03681]])
    assert np.allclose(vtb, expected_vtb, atol=1e-3)

    # Test BQCRB type 1
    bqcrb1 = BQCRB(
        all_parameter_values, prob_normalized, [], final_states, d_final_states, btype=1
    )
    expected_bqcrb1 = np.array([[45.48725379, 0.33691038], [0.33691038, 0.36839637]])
    assert np.allclose(bqcrb1, expected_bqcrb1, atol=1e-3)

    # Test BQCRB type 2
    bqcrb2 = BQCRB(
        all_parameter_values, prob_normalized, [], final_states, d_final_states, btype=2
    )
    expected_bqcrb2 = np.array([[10.542814, 0.0015452], [0.0015452, 0.29983117]])
    assert np.allclose(bqcrb2, expected_bqcrb2, atol=1e-3)
    ...
    expected_bqcrb3 = np.array([[1.39714369, -0.00959793], [-0.00959793, 0.25794208]])
    assert np.allclose(bqcrb3, expected_bqcrb3, atol=1e-3)
    ...
    expected_qvtb = np.array([[0.04371, 0.0], [0.0, 0.03529]])
    assert np.allclose(qvtb, expected_qvtb, atol=1e-3)
...
def test_bqfim_ldtype_single(LDtype):
    ...
    for xv in x_values:
        dynamics = Lindblad(
            time_span, rho0, hamiltonian_func(xv), d_hamiltonian_func(xv)
        )
        states, d_states = dynamics.expm()
        final_states.append(states[-1])
        d_final_states.append(d_states[-1])
    ...
    try:
        result = BQFIM(
            [x_values], prob_normalized, final_states, d_final_states, LDtype=LDtype
        )
    except ValueError as e:
        assert "does not exist" in str(e), f"{LDtype}: unexpected ValueError: {e}"
        return
...
def test_bqcrb_btype_single(btype):
    ...
    result = BQCRB(
        [x_values], prob_normalized, d_prob, final_states, d_final_states, btype=btype
    )
    assert np.isfinite(result)
...
def test_obb_smoke():
    ...
    for xv in x_values:
        ...
        states, d_states = dynamics.expm()
        d2_dynamics = Lindblad(
            time_span, rho0, hamiltonian_func(xv), d2_hamiltonian_func(xv)
        )
