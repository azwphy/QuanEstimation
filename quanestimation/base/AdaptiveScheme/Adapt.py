...
from quanestimation.base.Common.Common import extract_ele, SIC
from quanestimation.base.MeasurementOpt.MeasurementStruct import MeasurementOpt
from quanestimation.base.Parameterization.GeneralDynamics import Lindblad
from quanestimation.base.AsymptoticBound.CramerRao import QFIM, CFIM
...
def adaptive_dynamics(
    ...
    Iteratively updates the posterior distribution $p(x)$ and the tunable parameter
    $u$ based on experimental outcomes, using either the fix-optimal-point (FOP) or
    mutual-information (MI) strategy. The prior $p(x)$ is updated via Bayes' rule
    after each measurement.

    Args:
        x (list): Regimes of the parameters for the integral.
        p (ndarray): Prior distribution.
        M (list): POVM elements.
        tspan (array): Time points for the evolution.
        rho0 (matrix): Initial state (density matrix).
        H (list): Free Hamiltonians at each grid point in $x$.
        dH (list): Derivatives of the free Hamiltonian.
        decay (list): Decay operators and rates.
        Hc (list): Control Hamiltonians.
        ctrl (list): Control coefficients.
        W (matrix): Weight matrix for multi-parameter estimation.
        max_episode (int): Maximum number of episodes.
        eps (float): Machine epsilon.
        savefile (bool): If ``True``, save all posterior distributions; otherwise
            save only the final distribution and all estimates.
        method (str): ``"FOP"`` for fix-optimal-point or ``"MI"`` for mutual
            information.
        dyn_method (str, optional): ``"expm"`` (default) for matrix exponential or
            ``"ode"`` for differential equation solver.
    """
...
def adaptive_Kraus(x, p, M, rho0, K, dK, W, max_episode, eps, savefile, method):
    r"""
    Run adaptive quantum parameter estimation under Kraus operator dynamics.

    Iteratively updates the posterior distribution $p(x)$ and the tunable parameter
    $u$ based on experimental outcomes, using either the fix-optimal-point (FOP) or
    mutual-information (MI) strategy.
    ...
    Args:
        ...
        M (list): POVM elements.
        rho0 (matrix): Initial state (density matrix).
        K (list): Kraus operators at each grid point in $x$.
        dK (list): Derivatives of the Kraus operators.
        W (matrix): Weight matrix for multi-parameter estimation.
        max_episode (int): Maximum number of episodes.
        eps (float): Machine epsilon.
        savefile (bool): If ``True``, save all posterior distributions; otherwise
...
def iter_FOP_singlepara(p, p_num, x, u, rho_all, M, dim, x_opt, ei, res_exp=None):
    ...
    for a single unknown parameter.

    Updates the posterior distribution $p(x)$ via Bayes' rule using the
    experimental outcome, then sets the tunable parameter $u = x_{\rm opt} - \hat{x}$,
    where $x_{\rm opt}$ is the parameter value that maximizes the CFI.

    Args:
        p (ndarray): Current posterior distribution.
        p_num (int): Number of grid points.
        x (list): Regimes of the parameters.
        u (float): Current tunable parameter.
        rho_all (list): Pre-computed density matrices at all grid points.
        M (list): POVM elements.
        dim (int): Dimension of the Hilbert space.
        x_opt (float): The optimal (true) parameter value.
        ei (int): Current episode index.
        res_exp (int, optional): Experimental result. If None, prompts
            via input() for interactive use.

    Returns:
        tuple: (p, x_out, res_exp, u) where p is the updated posterior,
        x_out is the current estimate, res_exp is the experimental outcome,
        and u is the updated tunable parameter.
    """
    if res_exp is None:
    ...
    if (ei + 1) % 50 == 0:
        if (x_out + u) > x[0][-1] or (x_out + u) < x[0][0]:
...
def iter_FOP_multipara(
    ...
    Updates the multi-dimensional posterior distribution $p(\mathbf{x})$ via
    Bayes' rule and sets the tunable parameter vector
    $\mathbf{u} = \mathbf{x}_{\rm opt} - \hat{\mathbf{x}}$.

    Args:
        p (ndarray): Current posterior distribution.
        p_num (int): Total number of grid points.
        para_num (int): Number of unknown parameters.
        x (list): Regimes of the parameters.
        x_list (list): List of all grid-point coordinate tuples.
        u (list): Current tunable parameter vector.
        rho_all (list): Pre-computed density matrices at all grid points.
        M (list): POVM elements.
        dim (int): Dimension of the Hilbert space.
        x_opt (list): The optimal (true) parameter vector.
        ei (int): Current episode index.
        p_shape (tuple): Shape of the posterior array.
        res_exp (int, optional): Experimental result. If None, prompts
            via input() for interactive use.

    Returns:
        tuple: (p, x_out, res_exp, u) where p is the updated posterior,
        x_out is the current estimate vector, res_exp is the experimental
        outcome, and u is the updated tunable parameter vector.
    """
    if res_exp is None:
        res_exp = int(input("Please enter the experimental result: "))
    rho = [np.zeros((dim, dim), dtype=np.complex128) for i in range(p_num)]
...
def iter_MI_singlepara(p, p_num, x, u, rho_all, M, dim, ei, res_exp=None):
    ...
    Updates the posterior distribution $p(x)$ via Bayes' rule, then chooses the
    tunable parameter $u$ that maximizes the mutual information between the
    parameter and the measurement outcome.
...
def iter_MI_multipara(
    ...
    Updates the multi-dimensional posterior distribution $p(\mathbf{x})$ via
    Bayes' rule, then chooses the tunable parameter vector $\mathbf{u}$ that
...
def savefile_true(p, xout, y):
    r"""
    Save the posterior distribution, estimate, and experimental result for
    every iteration to CSV files (``pout.csv``, ``xout.csv``, ``y.csv``).

    Each call appends one row of data to the respective file.

    Args:
        p (ndarray): Posterior distribution at the current iteration.
...
def savefile_false(p, xout, y):
    r"""
    Save the final posterior distribution, all estimates, and all experimental
    results to CSV files (``pout.csv``, ``xout.csv``, ``y.csv``).

    In each call the entire arrays are written as a single row.

    Args:
        p (ndarray): Posterior distribution (final iteration only).
