...
import quanestimation.base.MeasurementOpt as Measure
from quanestimation.base.Common.Common import gramschmidt, sic_povm
...
class MeasurementSystem:
    ...
    def dynamics(
        self, tspan, rho0, H0, dH, Hc=None, ctrl=None, decay=None, dyn_method="expm"
    ):
        r"""
...
def MeasurementOpt(
    ...
    r"""
    Factory function for measurement optimization.

    Args:
        mtype (str, optional): Measurement optimization scenario.
            ``"projection"`` (default) -- Rank-one projective measurements;
            ``"input"`` -- Linear combination or rotation of given POVMs.
        minput (list, optional): Additional parameters for the ``"input"``
            scenario. Default ``[]``.
        ...
        method (str, optional): Optimization algorithm. Options are:
            ...
            ``"DE"`` (default) -- Differential evolution.
        **kwargs: Additional arguments passed to the selected algorithm class
            (see :class:`~quanestimation.MeasurementOpt.AD_Mopt`,
            :class:`~quanestimation.MeasurementOpt.PSO_Mopt`,
            :class:`~quanestimation.MeasurementOpt.DE_Mopt`).

    Returns:
        MeasurementSystem: An instance of the selected measurement optimization
        subsystem.
    """
...
def csv2npy_measurements(M, num):
    r"""
    Reshape a flat array of measurement operators into a list of arrays
    with ``num`` operators per segment and save as ``measurements.npy``.

    Each segment is reshaped into an $n \times n$ matrix (transposed).

    Args:
        M (ndarray): Flat array of measurement operator entries.
        num (int): Number of measurement operators per segment.
...
def load_measurements(M, num, indx=-1):
    r"""
    Load and reshape a flat array of measurement operators.

    Reshapes the flat array into a list of measurement segments with ``num``
    operators per segment, then returns the segment at the given index.

    Args:
        M (ndarray): Flat array of measurement operator entries.
        num (int): Number of measurement operators per segment.
        indx (int, optional): Index of the segment to return. Default ``-1``
            (the last segment).

    Returns:
        list: The selected segment of measurement operators (each reshaped to
        an $n \times n$ matrix, transposed).
    """
    n = int(np.sqrt(len(M[0])))
    N = int(len(M) / num)
    M_save = []
